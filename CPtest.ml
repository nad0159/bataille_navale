...
(**
Cette fonction est la version qui offre un message personnalisé dans le rapport de test si le résultat d'un test ne s'est pas terminé sur un [failwith]. L'argument doit être
le résultat d'un driver de test et vérifie si le test s'est terminé sur un [failwith]. 
{b ATTENTION: } une exception peut être levée que cela soit prévu ou non prévu. Il est donc important
que vous compreniez si c'était bien le comportement souhaité ou non.
...
@param res résultat d'un test    
*)
let assert_failwith_m(msg, res : string * 'a t_test_result) : unit =
  let step = List.hd(global_test_status.seq) in
  if is_current_test_valid(step) then
    begin 
      step.cassert <- step.cassert + 1;
      ...
      match t with
      | Test_fail_success(_) -> ()
      | _ -> step.error <- Some(Test_assert_violation(msg))
    end
  else step.cassert_ignored <- (step.cassert_ignored) + 1
;;
...
(**
Cette fonction est la version qui offre un message personnalisé dans le rapport de test si le résultat d'un test ne s'est pas terminé sur un [failwith]. 
Le premier argument est le message attendu levé par le [failwith], ainsi vous pouvez vérifier que c'est la bonne erreur qui a été remontée.
Le second argument doit être
le résultat d'un driver de test et vérifie si le test s'est terminé sur un [failwith]. 
{b ATTENTION: } une exception peut être levée que cela soit prévu ou non prévu. Il est donc important
...
@param res résultat d'un test    
*)
let assert_failwith_text_m(msg, text, res : string * string * 'a t_test_result) : unit =
  let step = List.hd(global_test_status.seq) in
  if is_current_test_valid(step) then
    begin 
      step.cassert <- step.cassert + 1;
      ...
      | Test_fail_success(m) -> (
        if m = text then
          ...
          step.error <- Some(Test_assert_violation(msg^", "^text^" <> "^m))
      )
      | _ -> step.error <- Some(Test_assert_violation(msg^", "^text))
    end
  else step.cassert_ignored <- (step.cassert_ignored) + 1
;;
